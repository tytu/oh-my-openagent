...
describe("sisyphus-task", () => {
  ...
  describe("CATEGORY_PROMPT_APPENDS", () => {
    test("visual-engineering category has design-focused prompt", () => {
      ...
      // #when / #then
      expect(promptAppend).toContain("视觉/UI")
      expect(promptAppend).toContain("设计优先的思维方式")
    })
    ...
    test("ultrabrain category has strategic prompt", () => {
      ...
      // #when / #then
      expect(promptAppend).toContain("业务逻辑")
      expect(promptAppend).toContain("战略顾问的思维方式")
    })
  })
  ...
  describe("CATEGORY_DESCRIPTIONS", () => {
    ...
    test("unspecified-high category exists and has description", () => {
      ...
      expect(description).toContain("工作量高")
    })
  })
  ...
  describe("category delegation config validation", () => {
    test("returns error when systemDefaultModel is not configured", async () => {
      ...
      expect(result).toContain("oh-my-opencode 需要配置默认模型")
    })
  })
  ...
  describe("resolveCategoryConfig", () => {
    ...
    test("returns default model from DEFAULT_CATEGORIES for builtin category", () => {
      ...
      expect(result!.promptAppend).toContain("视觉/UI")
    })
  ...
  describe("skills parameter", () => {
    test("skills parameter is optional - defaults to [] when not provided", async () => {
      // #given
      const { createDelegateTask } = require("./tools")
      ...
      // #then - should NOT throw, should proceed with empty skills
      await tool.execute(
        {
          description: "Test task",
          prompt: "Do something",
    ...
    test("null skills is normalized to [] (no throw)", async () => {
      // #given
      const { createDelegateTask } = require("./tools")
      let promptCalled = false
      ...
      // #when - null passed
      // #then - should NOT throw; null is normalized to []
      await tool.execute(
        {
          description: "Test task",
          prompt: "Do something",
    ...
    test("run_in_background is optional - defaults to false when not provided", async () => {
      // #given
      const { createDelegateTask } = require("./tools")
      let promptCalled = false
      ...
      // #when - run_in_background not provided (undefined)
      // #then - should NOT throw, should default to false (sync)
      await tool.execute(
        {
          description: "Test task",
          prompt: "Do something",
      ...
      // #then - sync mode should call prompt (not launch)
      expect(promptCalled).toBe(true)
    }, { timeout: 20000 })
  ...
  test("session_id with background=false should wait for result and return content", async () => {
    ...
    // #then - should contain actual result, not just "后台任务已继续"
    expect(result).toContain("This is the continued task result")
    expect(result).not.toContain("后台任务已继续")
  }, { timeout: 10000 })
  ...
  test("session_id with background=true should return immediately without waiting", async () => {
    ...
    expect(result).toContain("后台任务已继续")
  ...
  describe("buildSystemContent", () => {
    test("returns language instruction when no skills and no category promptAppend", () => {
      // #given
      ...
      // #then
      expect(result).toContain("<Language_Instruction>")
      expect(result).toBe(ZH_SUB_AGENT_INSTRUCTION)
    })

    test("returns skill content plus language instruction when skills provided without category", () => {
      // #given
      const { buildSystemContent } = require("./tools")
    ...
    test("returns category promptAppend plus language instruction when no skills", () => {
      // #given
      const { buildSystemContent } = require("./tools")
  ...
  test("combines skill content, category promptAppend, and language instruction with separators", () => {
    // #given
    const { buildSystemContent } = require("./tools")
    const skillContent = "You are a playwright expert"
    const categoryPromptAppend = "Focus on visual design"
    ...
    expect(result).toContain(ZH_SUB_AGENT_INSTRUCTION)
    expect(result).toBe(`${skillContent}\n\n${categoryPromptAppend}\n\n${ZH_SUB_AGENT_INSTRUCTION}`)
  })

  test("ZH_SUB_AGENT_INSTRUCTION contains inherited from main agent text", () => {
    // #then
    expect(ZH_SUB_AGENT_INSTRUCTION).toContain("已从主 agent 继承")
  })
})
