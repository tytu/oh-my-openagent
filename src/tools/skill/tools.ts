...
if (tools.length === 0 && resources.length === 0 && prompts.length === 0) {
    ...
    sections.push("")
    sections.push(`使用 \`skill_mcp\` 工具并传入 \`mcp_name="${serverName}"\` 来调用。`)
    sections.push("")
  }
...
export function createSkillTool(options: SkillLoadOptions = {}): ToolDefinition {
  ...
  return tool({
    ...
    args: {
      name: tool.schema.string().describe("来自available_skills的技能标识符（例如，'code-review'）"),
    },
...
if (skill.definition.agent && (!ctx?.agent || skill.definition.agent !== ctx.agent)) {
         throw new Error(`技能"${args.name}"仅对Agent"${skill.definition.agent}"可用`)
       }
