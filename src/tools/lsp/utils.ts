...
export function formatServerLookupError(result: Exclude<ServerLookupResult, { status: "found" }>): string {
   if (result.status === "not_installed") {
     ...
     return [
       ...
       `  ${installHint}`,
       ``,
       `支持的扩展名：${server.extensions.join(", ")}`,
       ``,
       `安装后，服务器将自动可用。`,
       `运行 'LspServers' 工具验证安装状态。`,
     ].join("\n")
   }

   return [
     `未针对扩展名 ${result.extension} 配置 LSP 服务器。`,
     ``,
     `可用服务器：${result.availableServers.slice(0, 10).join(", ")}${result.availableServers.length > 10 ? "..." : ""}`,
     ``,
     `要添加自定义服务器，请在 oh-my-opencode.json 中配置 'lsp'：`,
     `  {`,
     `    "lsp": {`,
     `      "my-server": {`,
...
export function formatSymbolInfo(symbol: SymbolInfo): string {
  ...
  const container = symbol.containerName ? ` (位于 ${symbol.containerName})` : ""
...
export function formatDiagnostic(diag: Diagnostic): string {
  ...
  return `${severity}${source}${code} 在 ${line}:${char}: ${diag.message}`
}
...
export function formatPrepareRenameResult(
  result: PrepareRenameResult | PrepareRenameDefaultBehavior | Range | null
   ...
   if (!result) return "无法在此位置重命名"

  // Case 1: { defaultBehavior: boolean }
  if ("defaultBehavior" in result) {
     return result.defaultBehavior ? "支持重命名（使用默认行为）" : "无法在此位置重命名"
  }
...
export function applyWorkspaceEdit(edit: WorkspaceEdit | null): ApplyResult {
   if (!edit) {
     return { success: false, filesModified: [], totalEdits: 0, errors: ["未提供编辑"] }
   }
      ...
      if ("kind" in change) {
        if (change.kind === "create") {
          try {
            ...
            result.success = false
            result.errors.push(`创建 ${change.uri}：${err}`)
          }
        } else if (change.kind === "rename") {
          try {
            ...
            result.success = false
            result.errors.push(`重命名 ${change.oldUri}：${err}`)
          }
        } else if (change.kind === "delete") {
          try {
            const filePath = uriToPath(change.uri)
            ...
            result.success = false
            result.errors.push(`删除 ${change.uri}：${err}`)
          }
        }
      } else {
        const filePath = uriToPath(change.textDocument.uri)
...
export function formatApplyResult(result: ApplyResult): string {
  ...
  if (result.success) {
     lines.push(`已对 ${result.filesModified.length} 个文件应用 ${result.totalEdits} 个编辑：`)
    ...
    if (result.filesModified.length > 0) {
       lines.push(`成功修改：${result.filesModified.join(", ")}`)
    }
  }
