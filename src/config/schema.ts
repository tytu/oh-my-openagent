...
export const ProfilingConfigSchema = z.object({
  ...
  slow_threshold_ms: z.number().default(100),
  memory_snapshot_interval: z.number().default(5),
  trace_api: z.boolean().default(true),
  trace_fileio: z.boolean().default(true),
  trace_polling: z.boolean().default(true),
})
...
export const BackgroundTaskConfigSchema = z.object({
  ...
  /** Stale timeout in milliseconds - interrupt tasks with no activity for this duration (default: 120000 = 2 minutes, minimum: 60000 = 1 minute) */
  staleTimeoutMs: z.number().min(60000).optional(),
  /** Maximum steps before auto-completion (0 = unlimited). Useful for explore agents to prevent runaway loops. */
  maxSteps: z.number().min(0).optional(),
  /** Maximum runtime in milliseconds before auto-completion (0 = unlimited). Per-task override of the global 30-min TTL. */
  maxRuntimeMs: z.number().min(0).optional(),
  /** Maximum duration of a single LLM step in milliseconds (default: 600000 = 10 minutes, 0 = unlimited). Interrupts slow-streaming stuck steps. */
  stepTimeoutMs: z.number().min(0).optional(),
})
