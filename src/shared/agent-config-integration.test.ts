...
describe("Agent Config Integration", () => {
  ...
  describe("Display name resolution", () => {
    test("returns correct display names for all builtin agents", () => {
      ...
      expect(displayNames).toContain("总编排")
      expect(displayNames).toContain("任务执行")
      expect(displayNames).toContain("战略规划")
      expect(displayNames).toContain("预规划")
      expect(displayNames).toContain("质量评审")
      expect(displayNames).toContain("技术诊断")
      expect(displayNames).toContain("知识检索")
      expect(displayNames).toContain("代码搜索")
      expect(displayNames).toContain("多元分析")
    })
    ...
    test("handles lowercase keys case-insensitively", () => {
      ...
      expect(displayNames[0]).toBe("总编排")
      expect(displayNames[1]).toBe("任务执行")
      expect(displayNames[2]).toBe("总编排")
      expect(displayNames[3]).toBe("任务执行")
      expect(displayNames[4]).toBe("战略规划")
      expect(displayNames[5]).toBe("战略规划")
    })
  ...
  describe("End-to-end config flow", () => {
    test("old config migrates and displays correctly", () => {
      ...
      // #then - display names are correct
      expect(sisyphusDisplay).toBe("总编排")
      expect(prometheusDisplay).toBe("战略规划")
