...
import { ZH_LANGUAGE_INSTRUCTION, ZH_SUB_AGENT_INSTRUCTION } from "../agents/utils";
    ...
    if (isSisyphusEnabled && builtinAgents.sisyphus) {
      ...
      if (plannerEnabled) {
        ...
        const prometheusBase = {
          ...
          description: `${configAgent?.plan?.description ?? "计划Agent"}（战略规划 - OhMyOpenCode）`,
      ...
      // Language constraint injection for Prometheus (idempotent)
      const prometheusAgent = agentConfig["prometheus"] as { prompt?: string } | undefined
      ...
      // Language constraint injection for Sisyphus-Junior (idempotent)
      // Note: Sisyphus-Junior's prompt_append is already processed by
      // buildSisyphusJuniorPrompt() inside createSisyphusJuniorAgentWithOverrides
      const juniorAgent = agentConfig["sisyphus-junior"] as { prompt?: string } | undefined
      if (juniorAgent?.prompt && !juniorAgent.prompt.includes("<Language_Instruction>")) {
        juniorAgent.prompt += ZH_SUB_AGENT_INSTRUCTION
      }

      // Language constraint injection for OpenCode-Builder (idempotent)
      const openCodeBuilderAgent = agentConfig["OpenCode-Builder"] as { prompt?: string } | undefined
