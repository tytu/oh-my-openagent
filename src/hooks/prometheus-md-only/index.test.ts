  ...
  afterEach(() => {
   ...
   describe("with Prometheus agent in message storage", () => {
    ...
    test("should inject workflow reminder when Prometheus writes to .sisyphus/plans/", async () => {
      ...
      // #then
      expect(output.message).toContain("PROMETHEUS 必须遵循的工作流提醒")
      expect(output.message).toContain("面谈")
      expect(output.message).toContain("METIS 咨询")
      expect(output.message).toContain("MOMUS 审查")
    })
    ...
    test("should inject read-only warning when Prometheus calls delegate_task", async () => {
      ...
      expect(output.args.prompt).toContain("不要修改任何文件")
    })
