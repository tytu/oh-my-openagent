    ...
    "tool.execute.before": async (
      ...
      if (!filePath) {
       ...
       if (!isAllowedFile(filePath, ctx.directory)) {
         ...
         throw new Error(
            `[${HOOK_NAME}] ${getAgentDisplayName("prometheus")} 只能写入/编辑 .sisyphus/ 目录下的 .md 文件。` +
            `尝试修改：${filePath}。` +
            `${getAgentDisplayName("prometheus")} 是只读规划者。使用 /start-work 来执行计划。` +
            `向用户道歉，提醒你的规划流程，告知你将按流程做什么，然后写入计划`
         )
       }
